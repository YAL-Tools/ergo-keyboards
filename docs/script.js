...
ColStagBoards.init = function(keyboards) {
	...
	kb.software = [type_Software.VIA,type_Software.Vial];
	kb.buildGuide = type_ValList.fromValue("https://github.com/foostan/crkbd/blob/main/docs/corne-cherry/v4/buildguide_en.md");
	...
	kb.extras = ["[v:Case] https://www.thingiverse.com/thing:6841200","[v:Modular case] https://www.thingiverse.com/thing:6717711","[v:Modular case] https://github.com/itsdmd/sofle-v1-modular-case/?tab=readme-ov-file"];
	var sofleV1 = add(kb);
	...
	kb.buildGuide = type_ValList.fromValue("https://josefadamcik.github.io/SofleKeyboard/build_guide_choc.html");
	...
	kb.caseType = type_ValList.fromValue(type_CaseType.Included);
	kb.firmware = type_ValList.fromValue(type_Firmware.QMK);
	kb.buildGuide = ["https://github.com/kata0510/Lily58/blob/master/Pro/Doc/buildguide_en.md"];
	ColStagKeyboard.setHotswap(kb,[type_SwitchProfile.MX,type_SwitchProfile.Choc],type_KeySpacing.MX);
	ColStagKeyboard.addSKBC(kb);
	add(kb);
	...
	kb.firmware = type_ValList.fromValue(type_Firmware.QMK);
	kb.software = [type_Software.Vial];
	ColStagKeyboard.setMatrix(kb,type_NumRangeList.fromSimpleArray([58,60]),type_NumRange.fromValue(6),type_NumRange.fromValue(4));
	...
	kb.firmware = [type_Firmware.Custom];
	kb.caseType = [type_CaseType.Included];
	...
	kb = { name : "Ursa Minor", img : type_ValList.fromValue("ursa-minor.webp"), thumbKeys : type_NumRange.fromValue(3), switchProfile : type_ValList.fromValue(type_SwitchProfile.Choc), keySpacing : [type_KeySpacing.MX], assembly : [type_Assembly.Handwired,type_Assembly.PCB], firmware : [type_Firmware.QMK], source : type_ValList.fromValue("https://github.com/markstory/ursa-minor-keyboard/")};
	kb.caseType = [type_CaseType.Included];
	ColStagKeyboard.setMatrix(kb,[type_NumRange.fromValue(54)],type_NumRange.fromValue(6),type_NumRange.fromValue(4));
	add(kb);
	kb = { name : "articulation80", thumbKeys : type_NumRange.fromValue(4), source : type_ValList.fromValue("https://github.com/mylestunglee/articulation80"), img : type_ValList.fromValue("articulation80.jpg"), firmware : [type_Firmware.QMK], notes : type_ValList.fromValue("Numpad/navigation blocks and the top row can be broken off.")};
	...
	kb.lighting = [type_Lighting.RGB,type_Lighting.Underglow];
	kb.ctlCount = type_NumRange.fromValue(2);
	kb.ctlFootprint = type_ValList.fromValue("Onboard/SMD");
	kb.ctlPinCount = type_NumRange.fromValue(0);
	kb.ctlName = type_ValList.fromValue("STM32G431");
	kb.web = ["!https://keeb.io/collections/iris-split-ergonomic-keyboard"];
	kb.kit = ["![v:Kit] https://keeb.io/products/iris-lm-kit","![v:Kailh PCB Kit] https://keeb.io/products/iris-lm-k-pcb-kit","![v:Gateron PCB Kit] https://keeb.io/products/iris-lm-g-pcb-kit"];
	kb.buildGuide = ["!https://docs.keeb.io/iris-lm-build-guide"];
	kb.prebuilt = ["!https://keeb.io/products/iris-lm"];
	kb.extras = ["[v:MagSafe Tenting Kit] https://keeb.io/products/magnetic-magsafe-tenting-stand-kit-for-split-keyboard-r2"];
	kb.notes = ["Kailh Choc V1 & Kailh Choc V2 are only supported on the Iris LM-K PCB.","Gateron LP KS-33 switches are only supported on the Iris LM-G PCB."];
	addIris(kb);
	...
	kb.caseType = type_ValList.fromValue(type_CaseType.Included);
	kb.firmware = [type_Firmware.Custom];
	kb.software = [type_Software.Custom];
	...
	kb.extras = ["https://www.thingiverse.com/thing:6962191"];
	ColStagKeyboard.addSKBC(kb);
	add(kb);
...
KeyboardTable.prototype = $extend(table_FancyTable.prototype,{
	...
	,initColConnection: function(kb) {
		...
		conType.filterNotes.set(type_Connection.WiredHalf,"For ZMK keyboards, one half can be connected to the computer (and communicate over USB)" + " while the other half talks to it over air");
		...
		conType.filterNotes.set(type_Connection.BluetoothWithCable,"Means that the keyboard connects to the computer over Bluetooth," + " but the halves talk to each other over wire");
		...
		conType.filterNotes.set(type_Connection.WirelessWithCable,"Same as \"" + conType.filterLabels.get(type_Connection.BluetoothWithCable) + "\", but for 2.4GHz/etc.");
	...
	,initLinks: function(kb) {
		this.addFilterHeader("Links").editorNotes.push(new table_FancyHeaderNote("Notes on link lists",table_FancyTableOnNotes.fromSimple(function(el) {
	...
	,initCuriosities: function(kb) {
		...
		light.defaultValue = [type_Lighting.RGB,type_Lighting.Simple];
		...
		asm.filterNotes.set(type_Assembly.Parametric,"Case and/or PCB are generated using a script" + " that allows to adjust key positions/count to some extent");
...
table_FancyTableFilters.build = function(table,out) {
	...
	while(_g < _g1.length) {
		...
		var togglesContainer = window.document.createElement("div");
		...
		var showIcon = window.document.createElement("span");
	...
	,buildEditor: function(out,store,restore) {
		...
		restore.push(function(kb) {
			...
			if(ranges == null || ranges.length == 0) {
				...
				while(_g < _g1) {
					var i = _g++;
					result[i] = type_NumRange.toString(_this[i]);
				}
...
var type_Assembly = $hxEnums["type.Assembly"] = { __ename__:true,__constructs__:null
	...
	,Parametric: {_hx_name:"Parametric",_hx_index:7,__enum__:"type.Assembly",toString:$estr}
};
type_Assembly.__constructs__ = [type_Assembly.Unspecified,type_Assembly.PCB,type_Assembly.ThroughHole,type_Assembly.Diodeless,type_Assembly.Handwired,type_Assembly.Adjustable,type_Assembly.Reversible,type_Assembly.Parametric];
